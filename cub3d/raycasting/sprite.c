...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/24 17:58:58 by seonggki          #+#    #+#             */
/*   Updated: 2021/03/24 21:45:16 by seonggki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		sprite_put_info(t_main *m, double *z_buffer)
{
	...
	while (++m->v.d_x < m->v.drawend_x)
	{
		...
		if (m->v.transform_y > 0 && m->v.d_x > 0 && m->v.d_x <
			m->w && m->v.transform_y < z_buffer[m->v.d_x])
		{
			...
			while (++m->v.d_y < m->v.drawend_y)
			{
				...
				{
					m->v.dst = m->img.data + (m->v.d_y *
						m->img.len + m->v.d_x * (m->img.bpp / 8));
