...
void ft_sprite_put_info(t_main *m, double *z_buffer)
{
	...
	{
		m->v.tex_x = (int)((m->v.d_x - (-m->v.sprite_w / 2 + m->v.sprite_screen_x)) * m->v.texture.w / m->v.sprite_w);
		if (m->v.transform_y > 0 && m->v.d_x > 0 && m->v.d_x < m->w && m->v.transform_y < z_buffer[m->v.d_x])
		{
			m->v.d_y = m->v.drawstart_y - 1;
			while (++m->v.d_y < m->v.drawend_y)
			{
				m->v.d = (m->v.d_y - m->v.v_movescreen) - m->h / 2 + m->v.sprite_h / 2;
				m->v.tex_y = m->v.d * m->v.texture.h / m->v.sprite_h;
				m->v.color = m->v.texture.data[64 * m->v.tex_y + m->v.tex_x];
				if ((m->v.color & 0x00FFFFFF) != 0)
				{
					m->v.dst = m->img.data + (m->v.d_y * m->img.len + m->v.d_x * (m->img.bpp / 8));
