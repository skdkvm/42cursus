...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/12 19:03:06 by seonggki          #+#    #+#             */
/*   Updated: 2022/01/12 19:03:50 by seonggki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/parser/parser.h"

void	no_dollar_word(char **p_command_line, size_t i, size_t j)
{
	int		k;
	char	*s;

	replace_dollar_word(p_command_line, "", (int)i, (int)j);
	s = *p_command_line;
	k = (int)i;
	k--;
	if (k < 0 || s[k] == ' ' || s[k] == '\t' || s[k] == ';' || s[k] == '|')
		skip_spases_tabs(p_command_line, ++k);
}
